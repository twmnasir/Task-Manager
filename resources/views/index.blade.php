...
<div class="max-w-3xl px-4 mx-auto mt-8 mb-4 bg-white">
    <h2 class="mb-4 text-2xl font-semibold text-center">Tasks List</h2>
    ...
    <ul id="item-list" class="space-y-4">
        ...
        <li data-title="{{ $task->title }}" class="flex items-center justify-between p-4 rounded-lg shadow-sm bg-gray-50">
            <span class="text-md">{{ ($tasks->currentPage() - 1) * $tasks->perPage() + $loop->index + 1 }}. {{ $task->title }}</span>
    ...
    <div class="flex my-3">
        <p class="inline-block">{{ $tasks->links() }}</p>
    </div>
</div>
@endsection
@section('js_footer')
